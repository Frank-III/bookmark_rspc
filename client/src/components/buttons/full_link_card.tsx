...
import { AlignJustify, Archive, ArchiveX, ArrowRight, Calendar, CheckCircle2, ChevronLeft, Copy, Menu, Tags } from 'lucide-react';
  ...
  return (
    ...
    <div className='items-start space-y-1 py-0.5'>
      ...
      <div className='inline-flex space-x-2 items-center'>
        <ArchiveX size={16} />
        {archived}
        <Badge variant='outline' className={cn('ring-0 shadow-none px-1 py-0.5 rounded-full bg-opacity-20 font-normal', archived ? 'bg-rose-500 text-rose-500': 'bg-emerald-500 text-emerald-500')}>{archived ? 'false' : 'true'}</Badge>
      </div>
      <div className='flex flex-row space-x-2 items-center'>
        <Tags size={16}/>
        <div className='inline-flex overflow-x-auto scrollbar-none space-x-[5px]'>
        {tags.map((tag) => (<TagBadge className='py-0.5 mb-1' tag={tag}/>))}
        </div>
      </div>
      <div className={cn('inline-flex space-x-2 items-center wrap gap-2', !description && 'text-gray-400')}>
